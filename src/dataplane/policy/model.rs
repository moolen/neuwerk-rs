...
impl PolicySnapshot {
    ...
    pub(crate) fn candidate_rule_indices_for_group(
        ...
        self.compiled_groups
            .get(group_idx)
            .map(|group| group.all.candidate_rule_indices(proto, dst_ip))
    }
...
impl CompiledSourceGroup {
    fn new(group: &SourceGroup) -> Self {
        Self {
            all: CompiledModeIndex::new(group),
        }
    }
}
...
impl CompiledModeIndex {
    fn new(group: &SourceGroup) -> Self {
        let tcp = CompiledRuleBucket::new(group, collect_matching_rule_indices(group, 6));
        let udp = CompiledRuleBucket::new(group, collect_matching_rule_indices(group, 17));
        let icmp = CompiledRuleBucket::new(group, collect_matching_rule_indices(group, 1));
        let any_other = CompiledRuleBucket::new(group, collect_any_other_rule_indices(group));

        let mut other = HashMap::new();
        for proto in collect_other_protocols(group) {
            other.insert(
                proto,
                CompiledRuleBucket::new(group, collect_matching_rule_indices(group, proto)),
            );
        }
...
fn collect_matching_rule_indices(group: &SourceGroup, proto: u8) -> Vec<usize> {
    group
        .rules
        .iter()
        .enumerate()
        .filter(|(_, rule)| rule.matcher.proto.matches(proto))
        .map(|(idx, _)| idx)
        .collect()
}
...
fn collect_any_other_rule_indices(group: &SourceGroup) -> Vec<usize> {
    group
        .rules
        .iter()
        .enumerate()
        .filter(|(_, rule)| matches!(rule.matcher.proto, Proto::Any))
        .map(|(idx, _)| idx)
        .collect()
}

fn collect_other_protocols(group: &SourceGroup) -> BTreeSet<u8> {
    group
        .rules
        .iter()
