...
The service-account token and CA certificate must allow the Neuwerk to list and watch the objects
you plan to select.
...
Write the singleton policy through the normal policy API:
...
After the write succeeds, verify:
...
This feature solves dynamic source resolution. It does not define how to deploy the Neuwerk itself
inside Kubernetes.
