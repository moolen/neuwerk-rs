...
Operationally, Neuwerk is meant to run adjacent to the cluster as dedicated network
infrastructure, not inside the cluster as another workload.
...
Compatibility depends on the gateway seeing source IPs that can still be resolved back to pod IPs.
Overlay designs that hide or rewrite the original pod source IP before traffic reaches the gateway
