...
description: Fixed listener ports and conditional protocol dependencies used by the Neuwerk.
order: 2
---
...
- `53` is part of the hostname-policy path and should be reachable from managed clients that are
  expected to use Neuwerk DNS.
- `9600` and `9601` should only be reachable between Neuwerk nodes participating in cluster
  formation.
