...
description: Reference for the appliance runtime YAML accepted at /etc/neuwerk/config.yaml.
order: 2
---
...
dns:
  ...
  upstreams:
    - 10.0.0.2:53
    - 10.0.0.3:53
  # Optional per-query upstream timeout before Neuwerk returns SERVFAIL.
...
| `dns.upstream_timeout_ms` | `2000` | Upstream resolvers are slow enough that Neuwerk should wait longer before returning `SERVFAIL` |
...
#### Policy Document Match-Mode Semantics

Neuwerk stores one always-active policy document under `policy.policy` via the HTTP API and replication state.
For that document model:

- top-level policy `mode` is not supported
- each `source_groups[]` entry must set `mode: audit | enforce`
- each rule `mode` is optional and acts only as an override of its source-group mode
- when rule `mode` is omitted, rule evaluation inherits the containing `source_group.mode`
...
| `integration.membership.auto_evict_terminating` | `true` | Disable only when you must avoid automatic terminating-instance membership eviction |
| `integration.membership.stale_after_secs` | `0` | Add a grace period before considering unseen instances stale for membership handling |
| `integration.membership.min_voters` | `3` | Tune the minimum healthy voters threshold used by membership handling |
