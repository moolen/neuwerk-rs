...
The schema reference tells you which fields exist. This page explains how the Neuwerk turns those
...
1. a top-level singleton policy document
...
Each source group also carries a required rollout mode:

- `enforce` applies normal allow and deny behavior
- `audit` evaluates matching denies but converts them into allow
...
- an optional `mode` of `enforce` or `audit`
- a `match` block

Rule mode and source-group mode do different jobs:

- source-group mode supplies the default rollout posture
- rule mode overrides that posture for the individual rule when needed
...
5. apply the effective mode from the source group or matching rule override
6. if no rule matches, use the source group's `default_action` if it has one
7. if no source group produces a decision, use the top-level `default_policy`
