...
- no policy resource and no partial policy-subobject resources
...
### Policy Is Not Terraform-Managed

The control plane still stores policy as one aggregate document, but the provider does not expose a
policy resource. Source groups and rules are not independent backend resources, so the provider
does not try to model them as standalone Terraform resources either.
...
Release artifacts are currently distributed through signed GitHub Releases. Manual installs are
expected to verify both the checksum file and its detached signature before placing the provider in
