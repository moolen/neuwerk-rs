...
resource "azurerm_user_assigned_identity" "neuwerk" {
...
resource "azurerm_orchestrated_virtual_machine_scale_set" "neuwerk" {
  ...
  source_image_id             = var.neuwerk_image_id != "" ? var.neuwerk_image_id : null
  tags                        = var.tags

  dynamic "source_image_reference" {
    for_each = var.neuwerk_image_id == "" ? [1] : []
    content {
  ...
  os_profile {
    custom_data = base64encode(templatefile("${path.module}/../../cloud-init/neuwerk.yaml.tmpl", {
      ...
      identity_client_id     = azurerm_user_assigned_identity.neuwerk.client_id
    }))
  ...
  identity {
    type         = "UserAssigned"
    identity_ids = [azurerm_user_assigned_identity.neuwerk.id]
  }
