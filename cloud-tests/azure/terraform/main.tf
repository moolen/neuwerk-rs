...
module "storage" {
  ...
  container_name       = "neuwerk"
  neuwerk_binary_path  = var.neuwerk_binary_path
  neuwerk_blob_name    = var.neuwerk_blob_name
  tags                 = var.tags
}
...
module "neuwerk_vmss" {
  source                       = "./modules/neuwerk_vmss"
  resource_group_name          = azurerm_resource_group.main.name
  location                     = azurerm_resource_group.main.location
  name_prefix                  = local.name_prefix
  instance_count               = var.neuwerk_instance_count
  vm_size                      = var.neuwerk_vmss_size
  ...
  dns_upstreams                = length(var.dns_upstreams) > 0 ? var.dns_upstreams : [for ip in module.upstream_vm.private_ips : "${ip}:53"]
  ...
  tags                         = var.tags
  neuwerk_image_id             = var.neuwerk_image_id
...
module "upstream_vm" {
  ...
  instance_count           = var.upstream_count
  resource_group_name      = azurerm_resource_group.main.name
  location                 = azurerm_resource_group.main.location
  name_prefix              = local.name_prefix
...
resource "azurerm_role_assignment" "neuwerk_blob_reader" {
  scope                = module.storage.account_id
  role_definition_name = "Storage Blob Data Reader"
  principal_id         = module.neuwerk_vmss.identity_principal_id
}

resource "azurerm_role_assignment" "neuwerk_network_contributor" {
  scope                = azurerm_resource_group.main.id
  role_definition_name = "Network Contributor"
  principal_id         = module.neuwerk_vmss.identity_principal_id
}

resource "azurerm_role_assignment" "neuwerk_reader" {
  scope                = azurerm_resource_group.main.id
