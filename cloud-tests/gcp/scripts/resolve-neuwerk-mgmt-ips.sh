...
FW_IGM=$(terraform output -json neuwerk_igm 2>/dev/null | jq -r '.name // empty')
MGMT_SUBNET=$(terraform output -raw mgmt_subnet_name 2>/dev/null || true)
popd >/dev/null

if [ -z "$PROJECT" ] || [ -z "$ZONE" ] || [ -z "$FW_IGM" ]; then
  echo "missing terraform outputs for project_id/zone/neuwerk_igm" >&2
  exit 1
fi
...
if [ -z "$instances" ]; then
  echo "no neuwerk instances found in MIG $FW_IGM" >&2
  exit 1
fi
