...
variable "traffic_architecture" {
  type        = string
  default     = "gwlb"
  description = "Traffic steering architecture: gwlb (GWLB+GENEVE) or eni_no_encap (direct routing to neuwerk ENI)."
...
variable "neuwerk_binary_path" {
  ...
  description = "Path to the Neuwerk binary uploaded to S3."
}

variable "neuwerk_ami_id" {
  type        = string
  default     = ""
  description = "Optional custom AMI ID for Neuwerk nodes only. Empty uses the default Ubuntu 24.04 AMI lookup."
}

variable "neuwerk_object_key" {
  type        = string
  default     = "neuwerk"
  description = "S3 object key for the Neuwerk binary."
}
...
variable "neuwerk_dpdk_runtime_object_key" {
  type        = string
  default     = "dpdk-runtime.tar.gz"
  description = "S3 object key for optional DPDK runtime bundle."
}

variable "neuwerk_instance_type" {
  type        = string
  default     = "c6in.large"
  description = "Neuwerk instance type (Intel, 2 vCPU target)."
}

variable "neuwerk_asg_min_size" {
  type        = number
  default     = 1
  description = "Minimum ASG size for neuwerk nodes (GWLB architecture)."
}

variable "neuwerk_asg_desired_capacity" {
  type        = number
  default     = 2
  description = "Desired ASG size for neuwerk nodes (GWLB architecture)."
}

variable "neuwerk_asg_max_size" {
  type        = number
  default     = 3
  description = "Maximum ASG size for neuwerk nodes (GWLB architecture)."
}

variable "neuwerk_asg_heartbeat_timeout_secs" {
  type        = number
  default     = 900
...
variable "consumer_instance_type" {
  type        = string
  default     = "c6in.large"
  description = "Consumer instance type (parity with neuwerk)."
}

variable "upstream_instance_type" {
  type        = string
  default     = "c6in.large"
  description = "Upstream instance type (parity with neuwerk)."
}
...
variable "neuwerk_snat_mode" {
  type        = string
  default     = "none"
  description = "SNAT mode for neuwerk dataplane."
}
