...
## Runtime Knob Documentation
- `docs/operations/runtime-knobs.md` is the canonical operator-facing reference for supported `/etc/neuwerk/config.yaml` runtime paths.
- Any change that adds, removes, renames, or changes the default or operator-visible behavior of a supported runtime config path must update `docs/operations/runtime-knobs.md` in the same change.
