...
compile_library()
{
	...
	compile_configure()
	{
		cd "$rootOutputFolderPath" 1>/dev/null 2>/dev/null
			...
			CFLAGS='-D_BSD_SOURCE -Dstat64=stat -Dfstat64=fstat -Dftruncate64=ftruncate -Dmmap64=mmap' ./configure --prefix=/usr --host="$configureHost" --disable-shared --enable-static --disable-dependency-tracking --disable-silent-rules --enable-fast-install
...
cargo_key_value_pairs()
{
	...
	cargo_key_value_pairs_link_lib 'static' 'numa'
