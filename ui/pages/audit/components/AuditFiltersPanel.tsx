...
export const AuditFiltersPanel: React.FC<AuditFiltersPanelProps> = ({
  ...
  >
    <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
      <div>
        ...
        <div className="mt-1 text-xs" style={{ color: "var(--text-muted)" }}>
          Filter persisted deny findings by type and source group before
          refreshing the cluster snapshot.
        </div>
      </div>
    ...
    <div className="grid grid-cols-1 gap-3 md:grid-cols-[minmax(10rem,13rem)_minmax(11rem,15rem)_auto]">
      <label className="text-sm" style={{ color: "var(--text-secondary)" }}>
        Finding type
        <select
          value={typeFilter}
          onChange={(e) =>
            onTypeFilterChange(e.target.value as AuditFindingType | "all")
          }
          disabled={disabled}
      ...
      <button
        ...
        style={{
          background:
            loading || disabled ? "var(--text-muted)" : "var(--accent)",
        }}
      >
