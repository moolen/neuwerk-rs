...
if bootstrap_path.exists():
    ...
    old_mode_block = """  data_mode=\"$(resolve_data_plane_mode)\"\n  data_selector=\"$(resolve_data_plane_selector \"$provider\" \"$data_iface\" \"$data_mode\")\"\n"""
    new_mode_block = """  data_mode=\"$(resolve_data_plane_mode)\"\n  if [[ \"$data_mode\" == \"dpdk\" && ! -d \"/sys/class/net/$data_iface\" ]]; then\n    echo \"unable to resolve dataplane interface\" >&2\n    exit 1\n  fi\n  data_selector=\"$(resolve_data_plane_selector \"$provider\" \"$data_iface\" \"$data_mode\")\"\n"""

    text = bootstrap_path.read_text(encoding="utf-8")
    updated = text
    if old_iface_check in updated:
        updated = updated.replace(old_iface_check, new_iface_check, 1)
    if old_mode_block in updated:
        updated = updated.replace(old_mode_block, new_mode_block, 1)
...
launch_old = """if [[ -f \"$env_file\" ]]; then\n  # shellcheck disable=SC1090\n  source \"$env_file\"\nfi\n"""
launch_new = """if [[ -f \"$env_file\" ]]; then\n  # shellcheck disable=SC1090\n  source \"$env_file\"\nfi\nif [[ -n \"${LD_LIBRARY_PATH:-}\" ]]; then\n  export LD_LIBRARY_PATH\nfi\nif [[ -n \"${RTE_EAL_PMD_PATH:-}\" ]]; then\n  export RTE_EAL_PMD_PATH\nfi\n"""
if launch_old in launch_text and "export LD_LIBRARY_PATH" not in launch_text:
    launch_path.write_text(launch_text.replace(launch_old, launch_new, 1), encoding="utf-8")
...
RUNTIME_ENV="${CONFIG_DIR}/runtime.env"
APPLIANCE_ENV="/etc/neuwerk/appliance.env"
...
cat >"${APPLIANCE_ENV}" <<APPLIANCE
# Generated by neuwerk-demo-configure for the Vagrant gateway demo.
NEUWERK_BOOTSTRAP_MANAGEMENT_INTERFACE=${MGMT_IFACE}
NEUWERK_BOOTSTRAP_DATA_INTERFACE=dp0
NEUWERK_BOOTSTRAP_DATA_PLANE_MODE=tun
NEUWERK_BOOTSTRAP_DEFAULT_POLICY=allow
NEUWERK_BOOTSTRAP_SNAT_MODE=${SNAT_IP}
NEUWERK_BOOTSTRAP_HTTP_BIND=${NEUWERK_VAGRANT_MGMT_IP}:8443
NEUWERK_BOOTSTRAP_METRICS_BIND=${NEUWERK_VAGRANT_MGMT_IP}:8080
NEUWERK_BOOTSTRAP_DNS_TARGET_IPS=${NEUWERK_VAGRANT_MGMT_IP}
NEUWERK_BOOTSTRAP_DNS_UPSTREAMS=1.1.1.1:53,8.8.8.8:53
